#!/usr/bin/env python3
"""
Predict age from PyHEARTS extracted ECG features using Autoage dataset.

This script:
1. Extracts age information from Autoage subject info CSV file
...
OUTPUT_DIR = PROJECT_ROOT / "results" / f"age_prediction_autoage_{timestamp}"
...
def extract_age_from_subject_info():
    """
    Extract age and sex information from Autoage subject info CSV.
    
    Returns:
        dict: Mapping from subject_id (4-digit string) to (age, sex) tuple
    """
    ...
    try:
        ...
        # Create mapping: subject_id -> (age, sex)
        age_sex_map = {}
        
        for _, row in df.iterrows():
            subject_id = str(int(row['ID'])).zfill(4)  # Format as 4-digit string (e.g., "0001")
            
            # Parse age range to get midpoint
            age_range = row.get('age_range', None)
            if pd.isna(age_range):
                continue
            
            # Parse range like "20-24" to get midpoint
            match = re.match(r'(\d+)-(\d+)', str(age_range))
            if match:
                low, high = int(match.group(1)), int(match.group(2))
                age = (low + high) / 2
            else:
                # If not a range, try to parse as single number
                try:
                    age = float(age_range)
                except ValueError:
                    continue
            
            # Get sex (convert to M/F format)
            sex_str = row.get('sex', None)
            if pd.isna(sex_str):
                sex = None
            else:
                sex_str = str(sex_str).strip()
                if sex_str.lower() in ['male', 'm', '1']:
                    sex = 'M'
                elif sex_str.lower() in ['female', 'f', '0']:
                    sex = 'F'
                else:
                    sex = None
            
            age_sex_map[subject_id] = (age, sex)
...
def build_dataset():
    ...
    # Load age/sex mapping from subject info
    print("Loading subject age/sex information...")
    age_sex_map = extract_age_from_subject_info()
    print(f"  Loaded age/sex info for {len(age_sex_map)} subjects")
    ...
    for subject in subjects:
        age_sex = age_sex_map.get(subject)
...
def evaluate_models(X, y, feature_names):
    ...
    # Define models - with larger dataset, can use more complex models
    models = {
...
def visualize_results(results, importance_df, output_dir):
    ...
    fig.suptitle('Age Prediction Model Performance (Autoage Dataset)', fontsize=16, fontweight='bold')
...
def main():
    print("="*80)
    print("Age Prediction from PyHEARTS Features (Autoage Dataset)")
    print("="*80)
    ...
    # Prepare features - with larger dataset, can use more features
    X, y, feature_names = prepare_features(df, use_feature_selection=True, n_features=100)
    ...
    importance_df = analyze_feature_importance(best_model, feature_names, X_scaled, y, top_n=50)
