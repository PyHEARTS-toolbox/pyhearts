"""
Record-level P/T detection via STPQ template windows (w1, w2).
...
P search (when ``record_stpq_p_r_anchor``): fixed PR window before ``current_r`` (q1c)
or ``next_r`` (legacy Rahul S→Q beat assignment), not S→Q fraction w2.
"""
...
def _tpl_index_to_sample(
    ...
    """Map STPQ template index (0=S … n-1≈Q) to a full-signal sample."""
    if n_tpl < 2:
        return int(s_i)
...
def _inverted_plateau_apex_forward(
    ...
    forward = _ms_before_r(sampling_rate, cfg.record_stpq_t_plateau_apex_forward_ms)
...
def _p_r_anchor(
    ...
    if not cfg.record_stpq_p_r_anchor:
        return None
    mode = getattr(cfg, "record_stpq_p_r_anchor_mode", "current_r")
    if mode == "current_r":
        return int(r_det)
...
def p_pr_window_samples(
    ...
    *,
    ...
    """
    p_lo = int(r_anchor) - _ms_before_r(sampling_rate, cfg.record_stpq_p_pr_max_ms)
    p_hi = int(r_anchor) - _ms_before_r(sampling_rate, cfg.record_stpq_p_pr_min_ms)
    p_lo = max(0, p_lo)
...
def _suppress_early_peak_beat_tuning(tmpl) -> bool:
    """
    sel301-class: landmark very early in the S→Q template — use wide STPQ + smooth,
...
def _early_peak_stpq_tuning_applies(tmpl) -> bool:
    """
    Beat-level early_peak STPQ for upright positive T (sel230 / sele0114).

    Not used in ``delineate_stpq_template``. sel301 (early landmark) and q2c defer
    (sel114) keep legacy per-beat / per-cycle behaviour instead.
    """
    if tmpl is None or not _is_early_peak_landmark(tmpl):
        return False
    if str(getattr(tmpl, "t_morphology", "normal") or "normal") != "normal":
...
def _stpq_t_search_end_tpl_idx(
    t_j: float,
    p_j: float,
    ...
    *,
    ...
    mode = getattr(cfg, "record_stpq_t_w1_end_mode", "mid_tp") if cfg is not None else "mid_tp"
    if mode == "template_tj_margin":
        if early_peak and cfg is not None:
            post = float(getattr(cfg, "record_stpq_t_early_peak_w1_post_tj_frac", 0.05))
        else:
            post = float(getattr(cfg, "record_stpq_t_w1_post_tj_frac", 0.15))
...
def _apply_w1_hi_sq_frac_floor(
    ...
    """
    Extend formula w1_hi when its S→Q fraction is below ``record_stpq_w1_hi_min_sq_frac``.

    Capped at ``p_j − record_stpq_w1_hi_pj_margin_sq_frac`` so the window stays clear of P.
    """
    if cfg is None or n_tpl < 2:
        return end_tpl
    min_frac = float(getattr(cfg, "record_stpq_w1_hi_min_sq_frac", 0.0) or 0.0)
    if min_frac <= 0.0:
        return end_tpl
    pj_margin = float(getattr(cfg, "record_stpq_w1_hi_pj_margin_sq_frac", 0.15))
    denom = float(n_tpl - 1)
...
def _stpq_t_window_samples(
    ...
    *,
    ...
    early = (
        ...
        and _early_peak_stpq_tuning_applies(tmpl)
    )
    narrow_w1 = early or (tmpl is not None and _t_search_prefer_negative(tmpl))
    end_tpl = _stpq_t_search_end_tpl_idx(t_j, p_j, cfg, early_peak=early)
    if not narrow_w1:
    ...
    if early:
        ...
        if t_hi - t_lo < 3:
            end_tpl = _stpq_t_search_end_tpl_idx(t_j, p_j, cfg, early_peak=False)
            end_tpl = _apply_w1_hi_sq_frac_floor(end_tpl, t_j, p_j, n_tpl, cfg)
            t_hi = _tpl_index_to_sample(s_i, q_next, end_tpl, n_tpl)
...
def _apply_stpq_t_rt_cap(
    ...
    if r_idx is not None and cfg.record_stpq_t_max_rt_ms > 0:
        rt_cap = int(r_idx) + _ms_before_r(sampling_rate, cfg.record_stpq_t_max_rt_ms)
...
def stpq_p_window_samples(
    *,
    ...
    """Beat-wise STPQ P search window (R-anchored PR or legacy S→Q w2)."""
...
def project_p_center_sample(
  ...
  Uses wavelet prior (R-relative samples), R-centered template offset, or STPQ landmark PR.
    """
...
def _resolve_stpq_t_tpl_idx_for_projection(tmpl, cfg: Optional[ProcessCycleConfig]) -> Optional[float]:
    """
    ...
    anchor STPQ at Tⱼ instead of the wide template-delineation apex.
    """
    if tmpl is None:
        return None
    if (
    ...
    src = getattr(cfg, "record_stpq_t_project_from", "delineated") if cfg is not None else "delineated"
    land = getattr(tmpl, "t_landmark_idx", None)
    off = getattr(tmpl, "t_offset_samples", None)
    if _early_peak_stpq_tuning_applies(tmpl) and land is not None:
        if src == "landmark":
            return float(land)
        if src == "delineated":
            ...
            return float(land)
        if src == "blend":
            blend = float(getattr(cfg, "record_stpq_t_landmark_blend_frac", 0.35)) if cfg else 0.35
...
def _search_p_template_guided(
    ...
    half = _ms_before_r(sampling_rate, cfg.record_stpq_p_template_guided_half_window_ms)
    ...
    penalty = float(cfg.record_stpq_p_template_guided_distance_penalty)
...
def defer_stpq_t_overwrite(
    ...
    *,
    ...
    q1c negative early_peak (sel301) uses legacy STPQ + smoothing instead.
    """
...
def stpq_t_use_biphasic_fallback(tmpl) -> bool:
...
def _search_t_biphasic_positive_lobe(
    ...
    early = _ms_before_r(sampling_rate, cfg.record_stpq_biphasic_pm_early_margin_ms)
    sep = _ms_before_r(sampling_rate, cfg.record_stpq_biphasic_pm_sep_before_neg_ms)
    late_cap = _ms_before_r(sampling_rate, cfg.record_stpq_biphasic_pm_late_cap_ms)
...
def _search_t_early_peak_apex(
    ...
    half = _ms_before_r(sampling_rate, cfg.record_stpq_t_template_guided_half_window_ms)
    ...
    late_cap = int(t_center) + _ms_before_r(
        sampling_rate,
        float(getattr(cfg, "record_stpq_t_early_peak_max_late_from_center_ms", 15.0)),
    )
    ...
    if not prefer_negative:
        edge_frac = float(
            getattr(cfg, "record_stpq_t_early_peak_rising_edge_frac", 0.35)
        )
        ...
        if rising is not None:
            idx, pol = rising
            max_dist = _ms_before_r(sampling_rate, cfg.record_stpq_t_mode1_max_dist_ms)
    ...
    max_dist = _ms_before_r(sampling_rate, cfg.record_stpq_t_mode1_max_dist_ms)
    min_amp_frac = float(cfg.record_stpq_t_mode1_min_amp_frac)
...
def _search_t_template_guided(
    ...
    if _early_peak_stpq_tuning_applies(tmpl):
    ...
    rel_center = float(t_center) - float(lo)
    penalty = float(cfg.record_stpq_t_template_guided_distance_penalty)
...
def _search_p_apex_in_window(
    ...
    *,
    ...
    if cfg.record_stpq_p_template_guided and p_center is not None:
        reconcile = _ms_before_r(sampling_rate, cfg.record_stpq_p_template_guided_reconcile_ms)
...
def _search_t_post_apex_dz_vs_positive_peak(
    ...
    late_cap = _ms_before_r(
        sampling_rate,
        float(getattr(cfg, "record_stpq_post_apex_dz_rt_tolerance_ms", 45.0)),
    )
...
def record_detect_t_peak(
    ...
    *,
    ...
    """T apex on delineation signal within STPQ window w1."""
    ...
    t_lo, t_hi = _apply_stpq_t_rt_cap(
    ...
    if (
        getattr(cfg, "record_stpq_post_apex_dz_preference", False)
    ...
    apex_mode = getattr(cfg, "record_stpq_t_apex_mode", "threshold")
    if _early_peak_stpq_tuning_applies(tmpl) and t_center is not None:
    ...
    if (
        apex_mode == "mode1"
        and cfg.record_stpq_t_template_guided
        and t_center is not None
    ):
    ...
    t_rel, t_pol = _apex_with_threshold(
        ...
        threshold_up=th_up,
        threshold_down=th_down,
        check_biphasic=stpq_t_use_biphasic_fallback(tmpl),
    )
    idx = lo + int(t_rel) if t_rel is not None else None

    if cfg.record_stpq_t_template_guided and t_center is not None and apex_mode != "mode1":
        reconcile = _ms_before_r(sampling_rate, cfg.record_stpq_t_template_guided_reconcile_ms)
...
def record_detect_p_peak(
    ...
    *,
    ...
    """P apex on delineation signal within PR-anchored or legacy STPQ w2 window."""
...
def record_fallback_p_search(
    ...
    *,
    ...
    """Template-guided P search in the PR window when STPQ misses."""
    if not cfg.record_stpq_p_r_anchor:
        return None
    ...
    if idx is None and cfg.record_stpq_p_template_guided:
...
def _p_anchor_mode(cfg: ProcessCycleConfig) -> str:
    return getattr(cfg, "record_stpq_p_r_anchor_mode", "current_r")
...
def _validate_p_guess(
    ...
    if cfg.record_stpq_p_r_anchor and mode == "current_r":
        pr_lo = float(r_det) - _ms_before_r(fs, cfg.record_stpq_p_pr_max_ms)
        pr_hi = float(r_det) - _ms_before_r(fs, cfg.record_stpq_p_pr_min_ms)
...
def record_stpq_pt_guesses(
    ...
    *,
    ...
    Returns (t_guess, p_guess) like legacy ``_stpq_pt_guesses``.
    """
    del scale
    if r_next is None:
        return None, None
    ...
    elif cfg.record_stpq_p_r_anchor and _p_anchor_mode(cfg) == "current_r":
        p_idx, _ = record_detect_p_peak(
            ecg_delim,
            len(ecg_delim),
            0,
    ...
    if p_guess is None and cfg.record_stpq_p_template_guided and cfg.record_stpq_p_r_anchor:
    ...
    max_rt = _ms_before_r(sampling_rate, cfg.record_stpq_t_max_rt_ms)
    if t_guess is not None:
        if t_guess <= float(r_det) + min_after_r or t_guess >= float(r_next) - min_after_r:
            t_guess = None
        elif cfg.record_stpq_t_max_rt_ms > 0 and t_guess > float(r_det) + max_rt:
            t_guess = None
